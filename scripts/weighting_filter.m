...
% Format coefficients to be compatibile with CMSIS IIR filter functions
num_sos = size(sos, 1);
coeffs = zeros(num_sos, 5);
for s=1:num_sos
coeffs(s, :) = [sos(s, 1:3) sos(s, 5:6)];
coeffs(s, 4:5) = -coeffs(s, 4:5);
endfor
coeffs
